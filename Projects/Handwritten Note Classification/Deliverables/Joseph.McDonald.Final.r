...
my.cols=c("red", "blue")
my.cols= c(rgb(1,0,0, 0.15), rgb(0,0,1,0.15))
...
plot(prin.comp.mod$scores[sample(c(1:1440),1440, replace=F), 2:3], pch=16, cex=.5,
     col=my.cols[(inde.vars[,1]=="CUR")+1], main="PCA Comp3 ~ Comp2 Class: Group")

#follow up notes from Dr. S on fixing overplotting
#dim(prin.comp.mod$scores[,2:3])
# #sample(c(1:1440),1440, replace=F)
# Over plotting
# -- use color blending
# --points are too dense
# --replace boxplots with violin plots
